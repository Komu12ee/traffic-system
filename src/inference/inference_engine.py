        ...
        for frame in self.video_loader.frames():
            ...
            try:
                ...
                if response.status_code == 200:
                    print("[INFO] Backend update successful")
                else:
                    print(f"[WARN] Backend responded with {response.status_code}: {response.text}")

            except Exception as e:
                print(f"[ERROR] Failed to send data to backend ({backend_url}): {e}")
...
# from src.inference.model_loader import YOLOModelLoader
# from src.alerts.alert_engine import AlertEngine
# from src.stream.video_loader import VideoLoader



# class InferenceEngine:
#     def __init__(self, stream_url, model_path="yolov8s.pt"):
